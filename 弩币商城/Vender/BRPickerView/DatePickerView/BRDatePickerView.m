...
// 根据万洋设计,小时和分钟在一起
@property (nonatomic,strong)NSArray * HourAndMinuteArr;
/** 显示类型 */
...
- (void)setupSelectDateFormatter:(BRDatePickerMode)model {
    switch (model) {
        ...
        case BRDatePickerModeCountDownTimer:
        {
            ...
            break;
        case BRWanYangDatePickerModeYMDHM:
...
- (void)initDefaultDateArray {
    ...
    if (self.showType == BRWanYangDatePickerModeYMDHM) {
        _hourIndex = 48;
    }
}
...
// 设置万洋那边的15分钟间隔
- (void)setupHourAndMinuteArr{
  NSMutableArray * array =   [NSMutableArray array];
    for (int i = 0; i < 24; i++) {
        for (int k = 0 ; k < 4; k++) {
            NSString * string = [NSString stringWithFormat:@"%02d:%02d",i,k * 15];
            [array addObject:string];
        }
    }
    self.HourAndMinuteArr = [array copy];
...
- (void)scrollToSelectDate:(NSDate *)selectDate animated:(BOOL)animated {
    ...
    if (self.showType == BRWanYangDatePickerModeYMDHM) {
        indexArr = @[@(yearIndex), @(monthIndex), @(dayIndex), @(48)];
    }
...
- (NSInteger)pickerView:(UIPickerView *)pickerView numberOfRowsInComponent:(NSInteger)component {
    ...
    if (self.showType == BRWanYangDatePickerModeYMDHM) {
           rowsArr = @[@(self.yearArr.count), @(self.monthArr.count), @(self.dayArr.count), @(self.HourAndMinuteArr.count)];
    }
    if (self.showType == BRDatePickerModeYMDHM) {
...
- (void)setDateLabelText:(UILabel *)label component:(NSInteger)component row:(NSInteger)row {
    if (self.showType == BRWanYangDatePickerModeYMDHM) {
        ...
        if (component == 0) {
            ...
            label.text = [NSString stringWithFormat:@"%@", self.HourAndMinuteArr[row]];
        }
...
- (NSDate *)getDidSelectedDate:(NSInteger)component row:(NSInteger)row {
    ...
    if (self.showType == BRWanYangDatePickerModeYMDHM) {
        ...
        selectDateValue = [NSString stringWithFormat:@"%@-%02ld-%02ld %@", self.yearArr[_yearIndex], [self.monthArr[_monthIndex] integerValue], [self.dayArr[_dayIndex] integerValue], self.HourAndMinuteArr[_hourIndex]];
